...
class KafkaProducer {
    ...
    @Autowired
    public KafkaProducer(LoggingUtils loggingUtils, CHKafkaProducer chKafkaProducer,
            @Value("${kafkaProducer.producerTimeout}") Long timeout) {
     ...
     * @param asyncResponseLogger RecordMetadata {@link Consumer} that can be implemented to allow the logging of the offset once
