...
public class FilingHistoryDescriptionProviderService {
    ...
    public FilingHistoryDescriptionProviderService(LoggingUtils loggingUtils,
            @Value("${filingHistory.dateFormat}") String dateFormat) {
    ...
    /**
     * Fetches the specified key from the filing history enumerations and replaces the variables with the ones in the
     * filingHistoryDescriptionValues. It strips out all asterisks in description. If descriptionKey is null, null is returned. If
     * the descriptionKey is not found in the filing history enumeration, the provided descriptionKey is returned. If
     * filingHistoryDescriptionValues is null the description found in filing history enumeration is returned (if available).
     *
    ...
    public String mapFilingHistoryDescription(String descriptionKey, Map<String, Object> filingHistoryDescriptionValues) {
        ...
        } else if (filingHistoryDescriptionValues.containsKey(FILING_HISTORY_DESCRIPTION_KEY)) {
            return (String) filingHistoryDescriptionValues.get(FILING_HISTORY_DESCRIPTION_KEY);
        } else {
